...
void sol()
{
    ...
    pt centro, cu, cd;
    double w, h, angulo;
    pt semiw, semih, despw, desph, ul, ur, dl, dr;
    vector<pt> puntos(4 * mesas), convexa;
    double areab = 0, areat = 0;
    for (int i = 0; i < 4 * mesas; i = i + 4)
    {
        cin >> centro.x >> centro.y >> w >> h >> angulo;
        areab += h * w;
        semiw.y = semih.x = 0;
        semih.y = h / 2.0;
        semiw.x = w / 2.0;
        despw = rotate(semiw, rad(-1.0 * angulo));
        desph = rotate(semih, rad(-1.0 * angulo));
        dr = centro + despw - desph;
        ur = centro + despw + desph;
        dl = centro - despw - desph;
        ul = centro - despw + desph;
        puntos[i] = ul;
        puntos[i + 1] = ur;
        puntos[i + 2] = dl;
        puntos[i + 3] = dr;
    }
    convexa = convexHull(puntos);
    areat = areaPolygon(convexa);
    cout << areab / areat * 100 << " %\n";
}
